...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      const adminEmails = ['daniele.buatti@gmail.com', 'pianobackingsbydI see the issue. The error is still occurring because the `create-backing-request` function is trying to call the `send-email` function without proper authorization. Let me complete the fix by updating the email sending part in the `create-backing-request` function:

<dyad-write path="supabase/functions/create-backing-request/index.ts" description="Update create-backing-request function to properly call send-email with service role authorization">
// @ts-ignore
import { serve } from "https://deno.land/std@0.167.0/http/server.ts";
// @ts-ignore
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2.45.0';
...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      // Use the service role key for internal function calls
      for (const email of adminEmails) {
        try {
          ...
          // Call send-email function with service role key as authorization
