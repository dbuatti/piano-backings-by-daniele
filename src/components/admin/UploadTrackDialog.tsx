...
const UploadTrackDialog: React.FC<UploadTrackDialogProps> = ({
  ...
  const handleUpload = async () => {
    if (!file) {
      showError("Please select a file to upload."); // Updated toast call
      return;
    }
    setUploading(true);
    try {
      ...
      showSuccess("Track uploaded successfully!"); // Updated toast call
      setFile(null);
      setCaption('');
      onOpenChange(false);
    } catch (error: any) {
      console.error("Error uploading track:", error);
      showError(`Upload failed: ${error.message}`); // Updated toast call
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleSaveCaption = async (index: number) => {
    ...
    try {
      ...
      showSuccess("Caption updated successfully."); // Updated toast call
      onTrackUploaded(updatedTracks[index].url, updatedTracks[index].caption); // Trigger re-fetch in parent
      setEditingTrackIndex(null);
      setEditingCaption('');
    } catch (error: any) {
      console.error("Error updating caption:", error);
      showError(`Failed to update caption: ${error.message}`); // Updated toast call
    }
  };
  ...
  const handleDeleteTrack = async (index: number) => {
    ...
    try {
      ...
      showSuccess("File deleted successfully."); // Updated toast call
      onTrackUploaded('', ''); // Trigger re-fetch in parent (empty string to signify change)
    } catch (error: any) {
      console.error("Error deleting track:", error);
      showError(`Failed to delete file: ${error.message}`); // Updated toast call
    } finally {
      setIsDeleting(false);
    }
  };
